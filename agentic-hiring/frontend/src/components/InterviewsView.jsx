...
import { Calendar, Clock, User, Check, Repeat, XCircle, Mail, Sparkles, Loader2, ClipboardList, Phone, Video } from "lucide-react"
...
export default function InterviewsView({ job }) {
  ...
  const fetchCandidates = async () => {
    ...
    try {
      ...
      const relevant = data.filter(c =>
        ["Shortlisted", "Interview Scheduled", "Interview Ready"].includes(c.status)
      )
  ...
  const currentSlot = selected ? schedule.find(ev => ev.candidate.trim().toLowerCase() === selected.trim().toLowerCase()) : null
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-4">
          <div className="feature-card">
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                ...
                {candidates.map(c => {
                  const slot = schedule.find(ev => ev.candidate.trim().toLowerCase() === c.name.trim().toLowerCase())
                          ...
                          {slot && (
                            <>
                              <Button
                                size="sm"
                                variant="ghost"
                                className="text-blue-300 hover:text-blue-200 hover:bg-blue-500/10 h-8 w-8 p-0"
                              ...
                              <Button
                                size="sm"
                                variant="ghost"
                                className="text-purple-300 hover:text-purple-200 hover:bg-purple-500/10 h-8 w-8 p-0"
                                onClick={(e) => {
                                  e.stopPropagation()
                                  setSelectedCandidateData(c)
          ...
          <div className="feature-card bg-slate-950/40 border border-slate-800">
            ...
            {(!selected) ? (
              <p className="text-slate-500">Select a candidate to view their schedule.</p>
            ) : schedule.filter(ev => ev.candidate.trim().toLowerCase() === selected.trim().toLowerCase()).length === 0 ? (
              <p className="text-slate-500">No interview scheduled for {selected}.</p>
            ) : (
              <div className="grid md:grid-cols-2 gap-3">
                {schedule
                  .filter(ev => ev.candidate.trim().toLowerCase() === selected.trim().toLowerCase())
                  .map(ev => (
